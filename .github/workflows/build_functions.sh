...
function package_name () {
	...
	if [[ "${PACKAGE_OS_ARCHITECTURE}" != "" ]]; then
		PACKAGE_OS_ARCHITECTURE_NAME="-${PACKAGE_OS_ARCHITECTURE}"
	else
		case $(uname -m) in
			aarch64|arm64)
				PACKAGE_OS_ARCHITECTURE_NAME='-arm64'
				;;
			x86_64)
				PACKAGE_OS_ARCHITECTURE_NAME='-amd64'
				;;
		esac
	fi
	...
	NEW_PACKAGE_NAME="Performous-${PACKAGE_OFFICIAL_VERSION}${PACKAGE_OS_NAME}${PACKAGE_OS_VERSION_NAME}${PACKAGE_OS_ARCHITECTURE_NAME}.${PACKAGE_SUFFIX}"
	NEW_PACKAGE_PATH="/tmp/${NEW_PACKAGE_NAME}"
	MASTER_NEW_PACKAGE_NAME="Performous-master[${PACKAGE_OFFICIAL_VERSION}]${PACKAGE_OS_NAME}${PACKAGE_OS_VERSION_NAME}${PACKAGE_OS_ARCHITECTURE_NAME}.${PACKAGE_SUFFIX}"
