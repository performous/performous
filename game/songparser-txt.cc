...
void SongParser::txtParseHeader(Song& song) {
	...
	{
		song.loadStatus = Song::LoadStatus::ERROR;
		std::clog << "songparser/error: Required MP3 '" << song.music[TrackName::BGMUSIC].string() << "' file isn't available." << std::endl;
	}
}
...
bool SongParser::txtParseField(Song& song, std::string const& line) {
	...
	else if (key == "MP3") song.music[TrackName::BGMUSIC] = absolute(value, song.path);
