...
void SpdLogger::initializeSinks(spdlog::level::level_enum const& consoleLevel) {
	...
	spdlog::filename_t filename = PathCache::getInstance().getLogFilename().u8string();
	spdlog::filename_t profilerLogFilename = PathCache::getInstance().getProfilerLogFilename().u8string();
...
void SpdLogger::toggleProfilerLogger() {
	...
	if (config["graphic/fps"].b() == true && std::find(m_sink->sinks().begin(), m_sink->sinks().end(), m_profilerSink) == m_sink->sinks().end())  {
		...
		notice(LogSystem::LOGGER, "Starting profiler... saving log at={}", PathCache::getInstance().getProfilerLogFilename());
	}
	else {
...
SpdLogger::~SpdLogger() {
	notice(LogSystem::LOGGER, "More details might be available in {}", PathCache::getInstance().getLogFilename().u8string());
	grabber.reset();
	spdlog::shutdown();
}
