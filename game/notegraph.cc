  ...
  m_notealpha(0.0f), m_nlTop(0.0, 4.0), m_nlBottom(0.0, 4.0), m_time()
{
	...
	m_nlTop.setTarget(m_song.vocals.noteMax, true);
	m_nlBottom.setTarget(m_song.vocals.noteMin, true);
	for (Notes::const_iterator it = m_song.vocals.notes.begin(); it != m_song.vocals.notes.end(); ++it)
		it->accuracy = 0.0; // Reset accuracy
	reset();
}

void NoteGraph::reset() {
	m_songit = m_song.vocals.notes.begin();
}
...
void NoteGraph::draw(double time, Database const& database, Position position) {
	...
	while (m_songit != m_song.vocals.notes.end() && (m_songit->type == Note::SLEEP || m_songit->end < time - (baseLine + 0.5) / pixUnit)) ++m_songit;

	// Automatically zooming notelines
	{
		int low = m_song.vocals.noteMax;
		int high = m_song.vocals.noteMin;
		int low2 = m_song.vocals.noteMax;
		int high2 = m_song.vocals.noteMin;
		for (Notes::const_iterator it = m_songit; it != m_song.vocals.notes.end() && it->begin < time + 15.0; ++it) {
	...
	for (Notes::const_iterator it = m_songit; it != m_song.vocals.notes.end() && it->begin < m_time - (baseLine - 0.5) / pixUnit; ++it) {
...
void NoteGraph::drawNotes() {
	// Draw note lines
	if (m_songit == m_song.vocals.notes.end() || m_songit->begin > m_time + 3.0) m_notealpha -= 0.02f;
