...
void NoteGraph::draw(float time, Database const& database, Position position) {
	...
	for (auto it = m_songit; it != m_vocal.notes.end() && it->begin < m_time - (baseLine - 0.5) / pixUnit; ++it) {
		...
		for (std::vector<Color>::const_iterator it_col = it->stars.begin(); it_col != it->stars.end(); ++it_col) {
			...
			float w = (it->end - it->begin) * pixUnit - m_noteUnit * 2.0; // width: including borders on both sides
			float hh = -m_noteUnit;
...
void NoteGraph::drawNotes() {
	// Draw note lines
	m_notelines.draw(Dimensions().stretch(dimensions.w(), (m_max - m_min - 13.0f) * m_noteUnit).middle(dimensions.xc()).center(dimensions.yc()), TexCoords(0.0, (-m_min - 7.0f) / 12.0f, 1.0f, (-m_max + 6.0f) / 12.0f));
	...
	for (auto it = m_songit; it != m_vocal.notes.end() && it->begin < m_time - (baseLine - 0.5) / pixUnit; ++it) {
		...
		switch (it->type) {
			...
			case Note::Type::RAP: //handle RAP notes like freestyle for now
			{
				...
				m_notebarfs.draw(dim, TexCoords(xoffset, 0.0, xoffset + dim.ar() / m_notebarfs.dimensions.ar(), 1.0f));
				if (alpha > 0.0) {
					float xoffset = rand() / float(RAND_MAX);
					m_notebarfs_hl.draw(dim, TexCoords(xoffset, 0.0, xoffset + dim.ar() / m_notebarfs_hl.dimensions.ar(), 1.0f));
				}
			}
			continue;
...
void NoteGraph::drawWaves(Database const& database) {
	...
	for (const Player& player: sortedPlayers) {
		if (player.m_vocal.name != m_vocal.name)
			continue;
		float const texOffset = 2.0f * m_time; // Offset for animating the wave texture
		Player::pitch_t const& pitch = player.m_pitch;
		size_t const beginIdx = std::max(0.0f, m_time - 0.5f / pixUnit) / Engine::TIMESTEP; // At which pitch idx to start displaying the wave
		...
		for (; idx < endIdx; ++idx, t += Engine::TIMESTEP) {
			...
			float val;
			if (hasNote && hasPrev) val = 0.5f * (noteIt->note + notePrev->note);
			...
			float thickness = clamp(1.0 + pitch[idx].second / 60.0f) + 0.5f;
			thickness *= NoteGraph::waveThickness() * (1.0f + 0.2f * std::sin(tex - 2.0f * texOffset)); // Further animation :)
