...
class TorrentDownloader::Impl {
  ...
  public:
	...
	void addTorrent(std::string url) {
		std::clog << "INFO:add torrent start" << std::endl;
		...
		std::clog << "INFO:path found="<<savePath.string() << std::endl;
		// fill torrent params
		error_code ec;
...
TorrentDownloader::TorrentDownloader(): self(new Impl) {
...
void TorrentDownloader::pause(bool state) { self->pause(state); }

void TorrentDownloader::pauseResume(std::string sha1) { self->pauseResume(sha1); }

void TorrentDownloader::addTorrent(std::string url) { self->addTorrent(url); }

void TorrentDownloader::removeTorrent(std::string sha1) { self->removeTorrent(sha1); }

std::vector<Torrent> TorrentDownloader::getTorrents() const { return self->getTorrents(); };

int TorrentDownloader::getUploadRate() const { return self->m_uploadRate; };

int TorrentDownloader::getDownloadRate() const { return self->m_downloadRate; };
