...
void SongParser::xmlParseHeader(Song& song) {
	// Parse notes.xml
	SSDom dom(m_ss);
...
/// Parse notes
void SongParser::xmlParse(Song& song) {
	// Parse notes.xml
	SSDom dom(m_ss);
...
Note SongParser::xmlParseNote(Song& song, xmlpp::Element const& noteNode, unsigned& ts) {
	Note n;
