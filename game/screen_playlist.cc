...
void ScreenPlaylist::enter() {
	...
	overlay_menu.close();
	getGame().loading(_("Loading song timestamps..."), 0.2);
	createSongListMenu();
	songlist_menu.open();
	reloadGL();
}
...
void ScreenPlaylist::createSongMenu(int songNumber) {
	...
	overlay_menu.add(MenuOption(firstOption, firstDesc).call([&, songNumber]() {
		...
		ScreenSing* ss = dynamic_cast<ScreenSing*>(s);
		assert(ss);
		ss->setSong(getGame().getCurrentPlayList().getSong(songNumber - 1));
		getGame().activateScreen("Sing");
	}));
	overlay_menu.add(MenuOption(_("Remove"), _("Remove this song from the list")).call([&, songNumber]() {
		// Minus 1 so it doesn´t remove #2 when you´ve selected #1
		getGame().getCurrentPlayList().removeSong(songNumber - 1);
		overlay_menu.close();
		if (getGame().getCurrentPlayList().isEmpty()) {
			getGame().activateScreen("Songs");
		} else {
	...
	if (songNumber >= 2) { //can't move up first song
		overlay_menu.add(MenuOption(_("Move up"), _("Move this song up the list")).call([&, songNumber]() {
			getGame().getCurrentPlayList().swap(songNumber -1, songNumber -2);
			createSongListMenu();
			overlay_menu.close();
		}));
	}
	int size = getGame().getCurrentPlayList().getList().size();
	if (songNumber < size) { //can't move down the last song
		overlay_menu.add(MenuOption(_("Move Down"), _("Move this song down the list")).call([&, songNumber]() {
			getGame().getCurrentPlayList().swap(songNumber -1, songNumber);
			createSongListMenu();
			overlay_menu.close();
		}));
	}
	overlay_menu.add(MenuOption(_("Back"), _("Back to playlist viewer")).call([&]() {
		overlay_menu.close();
	}));
}
