...
void ScreenIntro::populateMenu() {
	...
	m_menu.clear();
	auto perform = std::make_unique<MenuOption>(_("Perform"), _("Start performing!"), imgSing);
	perform->screen("Songs");
	m_menu.add(std::move(perform));
	auto practice = std::make_unique<MenuOption>(_("Practice"), _("Check your skills or test the microphones"), imgPractice);
	practice->screen("Practice");
	m_menu.add(std::move(practice));
	...
	auto configure = std::make_unique<MenuOption>(_("Configure"), _("Configure audio and game options"), imgConfig);
	configure->submenu(std::move(configmain));
	m_menu.add(std::move(configure));
	auto quit = std::make_unique<MenuOption>(_("Quit"), _("Leave the game"), imgQuit);
	quit->screen("");
