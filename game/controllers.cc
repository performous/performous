...
namespace {
	...
	NavButton navigation(Event const& ev) {
		#define DEFINE_BUTTON(dt, btn, num, nav) if ((input::DevType::DEVTYPE_##dt == input::DevType::DEVTYPE_GENERIC || ev.devType == input::DevType::DEVTYPE_##dt) && ev.button == dt##_##btn) return nav;
	...
	std::string buttonDebug(input::DevType type, Button b) {
		#define DEFINE_BUTTON(dt, btn, num, nav) if ((input::DevType::DEVTYPE_##dt == input::DevType::DEVTYPE_GENERIC || type == input::DevType::DEVTYPE_##dt) && b == dt##_##btn) \
		  return #dt " " #btn " (" #nav ")";
		#include "controllers-buttons.ii"
...
struct Controllers::Impl {
	...
	NameToButton m_buttons[static_cast<unsigned int>(DevType::DEVTYPE_N)];
	...
	Impl(): m_eventsEnabled() {
		#define DEFINE_BUTTON(devtype, button, num, nav) m_buttons[static_cast<unsigned>(input::DevType::DEVTYPE_##devtype)][#button] = devtype##_##button;
		#include "controllers-buttons.ii"
	...
	void parseControllers(xmlpp::DomParser const& dom, std::string const& path, SourceType sourceType) {
		...
		for (auto nodeit = n.begin(), end = n.end(); nodeit != end; ++nodeit) {
			...
			// Device type
			{
				...
				if (type == "guitar") def.devType = input::DevType::DEVTYPE_GUITAR;
				else if (type == "drumkit") def.devType = input::DevType::DEVTYPE_DRUMS;
				else if (type == "keytar") def.devType = input::DevType::DEVTYPE_KEYTAR;
				else if (type == "piano") def.devType = input::DevType::DEVTYPE_PIANO;
				else if (type == "dancepad") def.devType = input::DevType::DEVTYPE_DANCEPAD;
				else {
	...
	bool buttonByName(DevType type, std::string const& name, Button& button) {
		NameToButton const& n2b = m_buttons[static_cast<unsigned>(type)];
