...
static const float IDLE_TIMEOUT = 35.0f; // seconds
...
{
	m_songs.setAnimMargins(5.0f, 5.0f);
...
void ScreenSongs::update() {
	...
	float pstart = (!m_jukebox && song ? song->preview_start : 0.0f);
...
void ScreenSongs::prepare() {
	float time = m_audio.getPosition() - config["audio/video_delay"].f();
	if (m_video) m_video->prepare(time);
}

void ScreenSongs::drawJukebox() {
	float pos = m_audio.getPosition();
	float len = m_audio.getLength();
	float diff = len - pos;
...
void ScreenSongs::drawMultimedia() {
	if (!m_songs.empty()) {
		...
		float length = m_audio.getLength();
		float time = clamp(m_audio.getPosition() - float(config["audio/video_delay"].f()), 0.0f, length);
		m_songbg_default->draw();   // Default bg
		if (m_songbg.get() && !m_video.get()) {
			if (m_songbg->width() > 512 && m_songbg->dimensions.ar() > 1.1f) {
				// Full screen mode
				float s = sin(m_clock.get()) * 0.15f + 1.15f;
...
void ScreenSongs::drawCovers() {
	float spos = m_songs.currentPosition(); // This needs to be polled to run the animation
	...
	for (int i = -2; i < 6; ++i) {
		...
		float x = xtrans(0.0);
		float z = ztrans(0.0);
		float c = 0.4 + 0.6 * highlightf(0.0);
...
void ScreenSongs::drawMenu() {
	...
	if (cur->getComment() != "") {
		th.comment.dimensions.middle(0.0f).screenBottom(-0.12f);
