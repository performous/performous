...
void InstrumentGraph::drawMenu() {
	...
	else dimensions.screenBottom().middle().fixedWidth(std::min(m_width.get(), 0.5f));
	ThemeInstrumentMenu& th = *m_menuTheme;
	th.back_h.dimensions.fixedHeight(0.08f);
	m_arrow_up.dimensions.stretch(0.05f, 0.05);
	...
	for (MenuOptions::const_iterator it = m_menu.begin(); it != m_menu.end(); ++it, ++i) {
		...
		txt->draw(menutext);
		w = std::max(w, txt->w() + 2.0f * step + button_margin * 2.0f); // Calculate the widest entry
		y += step; // Move draw position down for the next option
	}
	// Draw comment text
	if (cur->getComment() != "") {
		//th.comment_bg.dimensions.middle().screenBottom(-0.2f);
		//th.comment_bg.draw();
