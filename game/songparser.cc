...
void SongParser::guessFiles () {
	...
	const std::vector<std::pair<fs::path*, char const*> > fields = {
		...
		{ &m_song.music[TrackName::PREVIEW], R"(^preview\.(mp3|m4a|ogg|aac)$)" },
		{ &m_song.music[TrackName::GUITAR], R"(^guitar\.(mp3|m4a|ogg|aac)$)" },
		{ &m_song.music[TrackName::BASS], R"(^(bass|rhythm)\.(mp3|m4a|ogg|aac)$)" },
		{ &m_song.music[TrackName::DRUMS], R"(^drums(_1)?\.(mp3|m4a|ogg|aac)$)" },
