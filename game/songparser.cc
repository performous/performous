...
void SongParser::parse(Song& song) {
	try {
		...
		if (song.loadStatus == Song::LoadStatus::HEADER) {
			...
			if (song.type == Song::Type::TXT) 
				txtParse(song);
			else if (song.type == Song::Type::INI)
				midParse(song);  // INI doesn't contain notes, parse those from MIDI
			else if (song.type == Song::Type::XML) 
				xmlParse(song);
...
void SongParser::guessFiles (Song& song) {
	...
	std::clog << "songparse/notice: Missing files for " << song.title << std::endl;
