...
void WebServer::StartServer(int tried) {
	if(tried > 2) {
		std::clog << "webserver/error: Couldn't start webserver tried 3 times." << std::endl;
		if(m_server) {
			m_server->stop();
		}
		return;
...
: m_songs(songs)
{
	if(config["game/webserver_access"].i() == 0) {
		...
		m_serverThread.reset(new boost::thread(boost::bind(&WebServer::StartServer, boost::ref(*this), 0)));
	}
}
