...
ScreenAudioDevices::ScreenAudioDevices(std::string const& name, Audio& audio): Screen(name), m_audio(audio) {
	...
	// FIXME: Create a dedicated icon for playback
	m_pdev_icon.reset(new Surface(getThemePath("sing_pbox.svg")));
...
void ScreenAudioDevices::manageEvent(SDL_Event event) {
	...
	if (nav != input::NONE) {
		...
		else if (nav == input::START) { save(); sm->activateScreen("Intro"); }
...
void ScreenAudioDevices::draw() {
	...
	m_theme->comment.draw(_("Use arrow keys to configure. Hit Enter/Start to save and test or Esc/Select to cancel."));
	// Additional info
...
void ScreenAudioDevices::save() {
	...
	// TODO: Check that all wanted devices were succesfully opened and if not, warn
}
