...
void ScreenPaths::manageEvent(input::NavEvent const& ev) {
	...
	if (ev.button == input::NavButton::NAV_CANCEL) {
		gm->activateScreen("Intro");
	}
	else if (ev.button == input::NavButton::NAV_PAUSE) m_audio.togglePause();
	else if (ev.button == input::NavButton::NAV_DOWN) m_menu.move(1); //one down
	else if (ev.button == input::NavButton::NAV_MOREDOWN) m_menu.move(5); //five down (page-dwon-key)
	else if (ev.button == input::NavButton::NAV_UP) m_menu.move(-1); //one up
	else if (ev.button == input::NavButton::NAV_MOREUP) m_menu.move(-5); //five up (page up key)
	else if (ev.button == input::NavButton::NAV_START) m_menu.action(); //enter: execute currently selected option.
}
