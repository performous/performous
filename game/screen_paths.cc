...
void ScreenPaths::generateMenuFromPath(fs::path path) {
	...
	if(showHiddenfolders) {
		auto hiddenToggle = std::make_unique<MenuOption>(_("Hide hidden folders"),_("Hide hidden folders"));
		hiddenToggle->call([this, sl, path]() {
			config["paths/showhiddenfolders"].b() = false;
			generateMenuFromPath(path);
		});
		m_menu.add(std::move(hiddenToggle));
	} else {
		auto hiddenToggle = std::make_unique<MenuOption>(_("Show hidden folders"),_("Show hidden folders"));
		hiddenToggle->call([this, sl, path]() {
	...
	if(folderInConfig) {
		auto removeFolder = std::make_unique<MenuOption>(_("Remove this folder"),_("Remove current folder from song folders"));
		removeFolder->call([this, sl, path, position]() {
		...
		m_menu.add(std::move(removeFolder));
	} else {
		auto addFolder = std::make_unique<MenuOption>(_("Add this folder"),_("Add current folder to song folders"));
		addFolder->call([this, sl, path]() {
			config["paths/songs"].sl().push_back(path.string()); //WHY the fuck is this const??
			generateMenuFromPath(path);
			//Reload internal, but that crashes!! rely on the user to press ctrl+r in song selection screen
		});
		m_menu.add(std::move(addFolder));
	}
	auto parent = path.parent_path();
	if (!parent.empty() && parent != path) {
		auto upFolder = std::make_unique<MenuOption>(_(".."),_("Go up one folder"));
		upFolder->call([this, sl, path]() {
					generateMenuFromPath(path.parent_path());
		});
		m_menu.add(std::move(upFolder));
	}
	...
	for (const auto &p : directories) {
		auto open = std::make_unique<MenuOption>(p.string(), _("Open folder"));
		open->call([this, p] { generateMenuFromPath(p); });
		m_menu.add(std::move(open));
	}
}
