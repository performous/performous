...
void Songs::reload_internal() {
	...
	Paths systemSongs = PathCache::getInstance().getPathsConfig("paths/system-songs");
	Paths paths = PathCache::getInstance().getPathsConfig("paths/songs");
...
Songs::Cache Songs::loadCache() {
	const fs::path songsMetaFile = PathCache::getInstance().getCacheDir() / SONGS_CACHE_JSON_FILE;
...
void Songs::CacheSonglist() {
	...
	fs::path cacheDir = PathCache::getInstance().getCacheDir() / SONGS_CACHE_JSON_FILE;
