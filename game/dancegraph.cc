...
void DanceGraph::setupJoinMenu() {
	...
	{ // Create difficulty opt
		...
		for (int level = 0; level < static_cast<int>(DanceDifficulty::DIFFICULTYCOUNT); ++level) {
...
std::string DanceGraph::getDifficultyString() const {
	return _(diffv[static_cast<int>(m_level)].c_str());
}

/// Get a string id for track and difficulty
std::string DanceGraph::getModeId() const {
	return m_gamingMode + " - " + diffv[static_cast<int>(m_level)] + (isKeyboard() ? " (kbd)" : "");
}

/// Attempt to change the difficulty by a step
void DanceGraph::changeDifficulty(int delta) {
	int newLevel = static_cast<int>(m_level) + delta;
	if(newLevel >= static_cast<int>(DanceDifficulty::DIFFICULTYCOUNT )|| newLevel < 0) return; // Out of bounds
...
void DanceGraph::engine() {
	...
	for (input::Event ev; m_dev->getEvent(ev); ) {
		...
		if (menuOpen() && ev.value != 0.0) {
			...
			else if (std::stoi(m_selectedDifficulty.so()) != static_cast<int>(m_level))
