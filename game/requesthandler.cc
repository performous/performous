...
RequestHandler::RequestHandler(std::string url, unsigned short port, Songs& songs, Game& game): m_songs(songs), m_game(game) {
	...
	if (!fs::exists(extensionsConfig)) {
		...
		try {
			...
			std::clog << std::string("webserver/error: file-extensions.json couldn't be copied due to: ") + e.what() << std::endl;
		}
	}
	m_contentTypes = readJSON(extensionsConfig);
}

std::string RequestHandler::getContentType(std::string const& extension) {
...
namespace {
	nlohmann::json SongsToJsonObject(Songs& m_songs) {
		nlohmann::json jsonRoot = nlohmann::json::array();
		SongCollection songs = m_songs.getSongs(true);
		for (std::shared_ptr<Song> const& song: songs) {
	...
	std::shared_ptr<Song> GetSongFromJSON(Songs& m_songs, nlohmann::json jsonDoc) {
	...
	auto RequestHandler::initWebserverRouter() {
		auto router = std::make_unique<Performous_Router_t>();

		router->http_get(
		R"(/)",
		[this] (auto request, auto) {
				return HandleFile(request, findFile("index.html"));
		});
		router->http_get(
			R"(/api/getDataBase.json)",
			[this] (auto request, auto) {
			...
			m_songs.setFilter({}, true);
			unsigned short sort = 1;
			...
			nlohmann::json jsonRoot = SongsToJsonObject(m_songs);
		...
		router->http_get(
			R"(/api/getCurrentPlayList.json)",
			[this] (auto request, auto) {
				nlohmann::json jsonRoot = nlohmann::json::array();
				for (std::shared_ptr<Song> const& song : m_game.getCurrentPlayList().getList()) {
					nlohmann::json songObject;
					songObject["Title"] = song->title;
					songObject["Artist"] = song->artist;
					songObject["Edition"] = song->edition;
		...
		router->http_get(
			R"(/api/getplaylistTimeout)",
			[this] (auto request, auto) {
				init_resp(request->create_response(restinio::status_ok()))
				  .set_body(std::to_string(config["game/playlist_screen_timeout"].ui()))
				  .done();
				return restinio::request_accepted();
			});
		...
		router->http_get(
			...
			[this] (auto request, auto params) {
				...
				return HandleFile(request, findFile(path));
		});
		...
		router->http_post(
			...
			try {
				...
				std::clog << std::string("webserver/error: JSON exception was thrown \"") + e.what() + std::string("\".") << std::endl;
			}
			...
			if (path == "api/add") {
				m_songs.setFilter(std::string(), true);
				std::shared_ptr<Song> songPointer = GetSongFromJSON(m_songs, jsonPostBody);
				if(!songPointer) {
			...
			if(m_game.getCurrentPlayList().isEmpty()) {
				...
				auto songIdToDelete = jsonPostBody["songId"].get<unsigned>();
				if(songIdToDelete >= 0) {
			...
			if(m_game.getCurrentPlayList().isEmpty()) {
				...
				try {
					auto songIdToMove = jsonPostBody["songId"].get<unsigned>();
					auto positionToMoveTo = jsonPostBody["position"].get<unsigned>();
					auto sizeOfPlaylist = static_cast<unsigned>(m_game.getCurrentPlayList().getList().size());
					if(songIdToMove < 0) {
		...
		router->http_put(R"-((.*))-", [this] (auto request, auto){
			init_resp(request->create_response(restinio::status_ok()))
			  .done();
			return restinio::request_accepted();
		});
		router->http_delete(R"-((.*))-", [this] (auto request, auto){
			init_resp(request->create_response(restinio::status_ok()))
			  .done();
			return restinio::request_accepted();
		});
		router->non_matched_request_handler(
			[] (auto request){
				std::clog << "webserver/notice: Non-matched request with body: " + request->body() << std::endl;
...
Performous_Server_Settings RequestHandler::make_server_settings(std::string const& url, unsigned short port) {
	auto settings = Performous_Server_Settings{config["webserver/threads"].ui()}
	.port (port)
	.address (url)
	.separate_accept_and_create_connect(true)
	.request_handler(initWebserverRouter())
	.read_next_http_message_timelimit(std::chrono::seconds(config["webserver/http_timelimit"].ui()))
	.write_http_response_timelimit(std::chrono::seconds(config["webserver/write_http_timelimit"].ui()))
	.handle_request_timeout(std::chrono::seconds(config["webserver/timeout"].ui()))
	.buffer_size(std::size_t(config["webserver/buffer_size"].ui()))
	.concurrent_accepts_count(config["webserver/threads"].ui())
	.max_pipelined_requests(config["webserver/request_pipeline"].ui())
	.acceptor_options_setter(
			[] (auto& options) {
				options.set_option(boost::asio::ip::tcp::acceptor::reuse_address(true));
			}).ip_blocker(m_blocker);
	return settings;
}

boost::asio::ip::address_v4 RequestHandler::getLocalIP(std::string const& service) {
...
restinio::request_handling_status_t RequestHandler::HandleFile(std::shared_ptr<restinio::request_t> request, fs::path const& filePath) {
